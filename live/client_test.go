...
func TestClientGetIfModified(t *testing.T) {
	...
	t.Run("first request no etag returns modified true and etag", func(t *testing.T) {
		...
		c := NewClient(WithBaseURL(srv.URL))
		var dst nbalive.ScoreboardResponse
	...
	t.Run("second request with etag returns 304 and leaves dst untouched", func(t *testing.T) {
		...
		c := NewClient(WithBaseURL(srv.URL))
		dst := nbalive.ScoreboardResponse{Scoreboard: nbalive.Scoreboard{Games: []nbalive.Game{{GameID: "sentinel"}}}}
...
func TestClientLiveGames(t *testing.T) {
	...
	c := NewClient(WithBaseURL(srv.URL))
	liveGames, err := c.LiveGames(context.Background())
	if err != nil {
		t.Fatalf("LiveGames() error = %v", err)
	}
	if len(liveGames) != 1 {
		t.Fatalf("len(live) = %d, want 1", len(liveGames))
	}
	if liveGames[0].GameID != "g2" {
		t.Fatalf("live[0].gameId = %q, want %q", liveGames[0].GameID, "g2")
	}
	if liveGames[0].GameStatus != nbalive.GameInProgress {
		t.Fatalf("live[0].gameStatus = %d, want %d", liveGames[0].GameStatus, nbalive.GameInProgress)
	}
}

...
import (
	...
	nbalive "github.com/darin-patton-hpe/nbalive"
)
		...
		client := NewClient(
			WithHTTPClient(&http.Client{
				Transport: roundTripFunc(func(r *http.Request) (*http.Response, error) {
					n := reqCount.Add(1)
					if n == 1 {
						return jsonResponse(200, "p1", nbalive.PlayByPlayResponse{
							Game: nbalive.PlayByPlayGame{GameID: testGameID, Actions: []nbalive.Action{
								{OrderNumber: 1}, {OrderNumber: 2}, {OrderNumber: 3},
							}},
						}), nil
					}
					return jsonResponse(200, "p2", nbalive.PlayByPlayResponse{
						Game: nbalive.PlayByPlayGame{GameID: testGameID, Actions: []nbalive.Action{
							{OrderNumber: 1}, {OrderNumber: 2}, {OrderNumber: 3},
		...
		client := NewClient(
			WithHTTPClient(&http.Client{
				Transport: roundTripFunc(func(r *http.Request) (*http.Response, error) {
					n := reqCount.Add(1)
					if n == 1 {
						return jsonResponse(200, "etag-pbp", nbalive.PlayByPlayResponse{
							Game: nbalive.PlayByPlayGame{GameID: testGameID, Actions: []nbalive.Action{{OrderNumber: 11}}},
						}), nil
					}
...
func TestWatchGameOverEmitsAndClosesChannel(t *testing.T) {
	synctest.Test(t, func(t *testing.T) {
			...
			WithHTTPClient(&http.Client{
				Transport: roundTripFunc(func(r *http.Request) (*http.Response, error) {
					...
					case r.URL.Path == "/playbyplay/playbyplay_"+testGameID+".json":
						return jsonResponse(200, "p1", nbalive.PlayByPlayResponse{
							Game: nbalive.PlayByPlayGame{GameID: testGameID},
						}), nil
					case r.URL.Path == "/boxscore/boxscore_"+testGameID+".json":
						return jsonResponse(200, "b1", nbalive.BoxScoreResponse{
							Game: nbalive.BoxScoreGame{GameID: testGameID, GameStatus: nbalive.GameFinal},
						}), nil
					default:
		...
		if ev.BoxScore == nil || ev.BoxScore.GameStatus != nbalive.GameFinal {
		...
		client := NewClient(
			WithHTTPClient(&http.Client{
				Transport: roundTripFunc(func(r *http.Request) (*http.Response, error) {
					...
					return jsonResponse(200, "p2", nbalive.PlayByPlayResponse{
						Game: nbalive.PlayByPlayGame{GameID: testGameID, Actions: []nbalive.Action{{OrderNumber: 21}}},
					}), nil
				}),
			}),
		)
